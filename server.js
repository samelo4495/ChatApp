...
async function initDb() {
    ...
    // MIGRAÇÃO: versões antigas da tabela rooms tinham config_json NOT NULL e não tinham owner_id.
    // Se detetarmos esse esquema, reconstruímos a tabela preservando os dados.
    const roomsCols = await dbAll(`PRAGMA table_info(rooms)`);
    const isOldSchema = roomsCols.some(c => c.name === "config_json");
    if (isOldSchema) {
        await dbRun(`ALTER TABLE rooms RENAME TO rooms_old`);
        await dbRun(`CREATE TABLE rooms (
            id TEXT PRIMARY KEY,
            name TEXT,
            protocol TEXT,
            owner_id TEXT,
        ...
        await dbRun(`INSERT INTO rooms (id, name, protocol, owner_id, target_id, created_at)
                     SELECT id, name, protocol, NULL, target_id, created_at FROM rooms_old`);
        await dbRun(`DROP TABLE rooms_old`);
        console.log("Tabela rooms migrada para o novo esquema.");
    }

    // TENTATIVA DE MIGRAR: Se a tabela já existia sem estas colunas, isto adiciona-as
    try {
        await dbRun(`ALTER TABLE rooms ADD COLUMN target_id TEXT`);
    } catch(e) { /* Coluna já existe, ignorar erro */ }
    try {
        await dbRun(`ALTER TABLE rooms ADD COLUMN owner_id TEXT`);
    } catch(e) { /* Coluna já existe, ignorar erro */ }
...
// ---------- Auth Middleware ----------
const requireAuth = (req, res, next) => req.session?.user ? next() : res.status(401).json({ error: "Não autorizado" });

// Caminho do socket de cada sala (Unix socket em Linux, named pipe em Windows)
function socketPathFor(roomId) {
    return process.platform === "win32"
        ? `\\\\.\\pipe\\chatapp_${roomId}`
        : path.join(DATA_DIR, `${roomId}.sock`);
}

// bwrap (sandbox) só existe em Linux; noutros sistemas corre o chatConnect diretamente
let BWRAP_AVAILABLE = null;
function hasBwrap() {
    if (BWRAP_AVAILABLE === null) {
        try {
            execSync("which bwrap", { stdio: "ignore" });
            BWRAP_AVAILABLE = process.platform === "linux";
        } catch (e) {
...
app.post("/api/rooms", requireAuth, async (req, res) => {
    ...
    try {
        ...
        // 4. Salas de IA precisam da instância chatConnect a correr
        if (protocol !== "p2p" && protocol !== "direct") {
            launchChatInstance(roomId);
        }
...
// Rota para o utilizador mudar a própria password
app.post("/api/password", requireAuth, async (req, res) => {
    const { oldPass, newPass } = req.body;
    if (!oldPass || !newPass) return res.status(400).json({ error: "Campos em falta" });

    try {
        const user = await dbGet(`SELECT * FROM users WHERE id = ?`, [req.session.user.id]);
        if (!user || !(await bcrypt.compare(oldPass, user.pass_hash))) {
            return res.status(401).json({ error: "Password atual incorreta" });
        }
        const hash = await bcrypt.hash(newPass, 10);
        await dbRun(`UPDATE users SET pass_hash = ? WHERE id = ?`, [hash, user.id]);
        res.json({ ok: true });
    } catch (e) {
...
app.post('/api/message', requireAuth, async (req, res) => {
    ...
    try {
        ...
        // 3. SE FOR IA, envia para o socket da instância desta sala
        const socketPath = socketPathFor(roomId);
        ...
        client.setTimeout(30000, () => {
            if (!res.headersSent) res.status(504).json({ error: "A IA demorou demasiado a responder" });
            client.destroy();
        });
...
app.delete("/api/rooms/:id", requireAuth, async (req, res) => {
    ...
    if (room?.proc) {
        try { room.proc.kill(); } catch (e) { /* processo já terminou */ }
    }
...
function syncAIBrain() {
    try {
        const stdout = execSync("lxc list --format csv -c n,s", { stdio: ["ignore", "pipe", "ignore"] }).toString();
        ...
        // LXC não existe neste sistema (ex: Windows) — escreve um placeholder uma única vez
        if (!fs.existsSync(AI_STATUS_FILE)) {
            fs.writeFileSync(AI_STATUS_FILE, "LXC indisponível neste sistema.\n");
        }
    }
}
...
function launchChatInstance(roomId) {
    ...
    // Remove socket antigo se existir (named pipes no Windows não ficam no disco)
    ...
    if (hasBwrap()) {
        ...
        // Sem bwrap (ex: Windows): corre a instância diretamente, sem sandbox
        proc = spawn(process.execPath, [scriptPath, socketPath], {
            cwd: roomDir,
            stdio: 'inherit'
        });
    }
    ...
    proc.on("error", (err) => {
        console.error(`Erro ao lançar instância da sala ${roomId}:`, err.message);
    });

    const room = activeRooms.get(roomId);
    if (room) room.proc = proc;
}
...
async function boot() {
    ...
    for (const r of rooms) {
        ...
        // Só as salas de IA precisam de instância própria; p2p é só ficheiro de log
        if (r.protocol !== "p2p" && r.protocol !== "direct") {
